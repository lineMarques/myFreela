<section>
    <form action="{{ route('curriculo.update') }}" method="post">
        ...
        <div class="mt-4">
            ...
            <x-text-input id="cep" class="w-1/4 block mt-0" type="text" name="cep" :value="old('cep', $user->address->cep)" required
                autocomplete="cep" />
        ...
        <div class="space-x-4 grid grid-cols-2">
            <div class="mt-4">
                ...
                <x-text-input id="road" class="w-full block mt-1" type="text" name="road" :value="old('road', $user->address->road)"
                    required autocomplete="road" />
                <x-input-error :messages="$errors->get('road')" class="mt-2" />
            </div>
            <div class="mt-4">
                <x-input-label for="number" :value="__('Numero')" />
                <x-text-input id="number" class="w-auto block mt-1" type="text" name="number " :value="old('number', $user->address->number)"
        ...
        <div class="space-x-4 grid grid-cols-2">
            <div class="mt-4">
                ...
                <x-text-input id="neighborhood" class="w-full block mt-1" type="text" name="neighborhood"
                    :value="old('neighborhood', $user->address->neighborhood)" required autocomplete="neighborhood" />
                <x-input-error :messages="$errors->get('neighborhood')" class="mt-2" />
            </div>
            <div class="mt-4">
                <x-input-label for="city" :value="__('Cidade')" />
                <x-text-input id="city" class="w-auto block mt-1" type="text" name="city" :value="old('city', $user->address->city)"
